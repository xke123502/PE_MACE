...
PairMACE::PairMACE(LAMMPS *lmp) : Pair(lmp)
{
  no_virial_fdotr_compute = 1;
   // Initialize field flags and values (*)
...
void PairMACE::compute(int eflag, int vflag)
{
  ...
  // Add uniform field data if present (*)  
  if (has_scalar_field) {
    auto field_scalar_full = torch::empty({n_nodes}, torch_float_dtype);
    field_scalar_full = field_scalar.repeat({n_nodes}).to(torch_float_dtype).to(device);
...
{ // Modified: 2026-01-06 by jwzhou - Fixed narg check for scalar field support
  // Maximum arguments: no_domain_decomposition (1) + scalar (1) + value (1) = 3
  if (narg > 3) {
    error->all(FLERR, "Too many pair_style arguments for pair_style mace.");
  }
  int current_arg = 0;
  // Check for no_domain_decomposition
  if (narg > 0 && strcmp(arg[0], "no_domain_decomposition") == 0) {
    domain_decomposition = false;
    current_arg++;
  }
  
  // Process remaining arguments for fields
  while (current_arg < narg) {
    std::string field_type(arg[current_arg]);
    current_arg++;
    if (field_type == "scalar") {
      if (current_arg >= narg) error->all(FLERR, "Missing scalar field value");
      field_scalar = torch::tensor({std::stod(arg[current_arg])});
      has_scalar_field = true;
      current_arg++;

    } else {
      error->all(FLERR, "Unknown field type. Use 'scalar'");
    }
  }
}
