...
class Configuration:
    ...
    # Use the new type definitions here
    field_scalar: Optional[FieldScalar] = None  # [n_atoms,]
    ...
    for atoms in atoms_list:
        all_configs.append(
            config_from_atoms(
                ...
                field_scalar_key=field_scalar_key,
                head_key=head_key,
...
def config_from_atoms(
    ...
    # Add these lines to extract field_scalar
    field_scalar = atoms.arrays.get(field_scalar_key, None)
...
def load_from_xyz(
    ...
    field_scalar_key: str = "field_scalar",
    head_key: str = "head",
...
def save_dataset_as_HDF5(dataset: List, out_name: str) -> None:
    with h5py.File(out_name, "w") as f:
        for i, data in enumerate(dataset):
            ...
            grp["head"] = data.head
            if data.field_scalar is not None:
                grp["field_scalar"] = data.field_scalar
...
def save_configurations_as_HDF5(configurations: Configurations, _, h5_file) -> None:
    ...
    for j, config in enumerate(configurations):
        ...
        subgroup["field_scalar"] = write_value(config.field_scalar)
