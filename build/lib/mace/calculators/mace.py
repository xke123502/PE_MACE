...
class MACECalculator(Calculator):
    ...
    args:
        ...
        model_type: str, type of model to load
                    Options: [MACE, DipoleMACE, EnergyDipoleMACE, MACE_Field]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.scalar_field_irreps = scalar_field_irreps
        ...
        if model_type == "MACE":
            ...
            raise ValueError(
                f"Give a valid model_type: [MACE, DipoleMACE, EnergyDipoleMACE, MACE_Field], {model_type} not supported"
            )
        ...
        if len(model_paths) > 1:
            ...
            if model_type in ["MACE", "EnergyDipoleMACE", "MACE_Field"]:
    ...
    def _create_result_tensors(
        ...
        if model_type == "MACE_Field":
            field_response = torch.zeros(num_models, num_atoms, 3, device=self.device)
            dict_of_tensors.update({"field_response": field_response})
        return dict_of_tensors

    def _atoms_to_batch(self, atoms):
        config = data.config_from_atoms(atoms, charges_key=self.charges_key)
        # add external field processing
        self.scalar_field_key = "field_scalar"        
        if self.scalar_field_key in atoms.arrays:
            config.field_scalar = atoms.arrays[self.scalar_field_key]
    ...
    # ensure external field data exists in batch
        if self.model_type == "MACE_Field":
    # check external field data
            if  self.scalar_field_key not in batch_base:
                raise ValueError("For MACE_Field, scalar field must be provided in the atoms object.")

        if self.model_type in ["MACE", "EnergyDipoleMACE", "MACE_Field"]:
